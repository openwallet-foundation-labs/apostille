...
describe('Workflow UI enabledWhen gating and local context set', () => {
  ...
  const createRootAgent = async () => {
    const agent = new Agent({
      config: { label: `wf-enable-root-${Date.now()}`, logger: new ConsoleLogger(LogLevel.off), walletConfig: { id: `wf-enable-root-${Date.now()}`, key: 'key' }, endpoints: ['rxjs:root'] },
  ...
  const gatingTemplate = (id: string) => ({
    template_id: id,
    version: '1.0.0',
    title: id,
    ...
    transitions: [
      { from: 'menu', to: 'menu', on: 'mark_ready', action: 'set_ready' },
      { from: 'menu', to: 'done', on: 'go', guard: 'context.ready == `true`' },
    ],
    catalog: {},
    actions: [ { key: 'set_ready', typeURI: 'https://didcomm.org/workflow/actions/state:set@1', staticInput: { merge: { ready: true } } } ],
    display_hints: {
      states: {
        menu: [
          { type: 'button', label: 'Mark Ready', event: 'mark_ready' },
          { type: 'button', label: 'Go', event: 'go', enabledWhen: 'context.ready == `true`' },
        ],
      },
    },
  })

  test('enabledWhen toggles based on context', async () => {
    ...
    try {
      ...
      const tplId = 'wf-enabled'
      await request(app).post('/api/workflows/templates').set('x-tenant-id', tenantA.id).send({ template: gatingTemplate(tplId) }).expect(200)
      ...
      // Initially 'Go' should be disabled per enabledWhen
      const st1 = await request(app).get(`/api/workflows/instances/${encodeURIComponent(instanceId)}`).set('x-tenant-id', tenantA.id).expect(200)
      const goItem1 = (st1.body?.status?.ui || []).find((i: any)=> i?.type==='button' && i?.event==='go')
      expect(goItem1?.disabled).toBeTruthy()

      // Mark Ready
      await request(app).post(`/api/workflows/instances/${encodeURIComponent(instanceId)}/advance`).set('x-tenant-id', tenantB.id).send({ event: 'mark_ready' }).expect(200)

      // Now Go enabled
      const st2 = await request(app).get(`/api/workflows/instances/${encodeURIComponent(instanceId)}`).set('x-tenant-id', tenantA.id).expect(200)
      const goItem2 = (st2.body?.status?.ui || []).find((i: any)=> i?.type==='button' && i?.event==='go')
      expect(goItem2?.disabled).toBeFalsy()

      // Advance Go
      await request(app).post(`/api/workflows/instances/${encodeURIComponent(instanceId)}/advance`).set('x-tenant-id', tenantB.id).send({ event: 'go' }).expect(200)
      const st3 = await request(app).get(`/api/workflows/instances/${encodeURIComponent(instanceId)}`).set('x-tenant-id', tenantA.id).expect(200)
      expect(st3.body?.status?.state).toBe('done')
    } finally {
