...
export default function SigningGuidedView({
  ...
  const handleFieldClick = useCallback(
    (field: SigningField) => {
      if (field.type === 'signature' || field.type === 'initials') {
        ...
        setCompletions((prev) => ({ ...prev, [field.id]: signerName }))
      }
    },
    [adoptedSignature, adoptedInitials, signerName]
  )
  ...
  const handleFinish = useCallback(async () => {
    setStamping(true)
    try {
      const stamped = await stampSignaturesOnPdf(pdfBytesForStamping, fields, completions, signerName)
  ...
  }, [pdfBytesForStamping, fields, completions, signerName, onComplete])
  ...
  return (
    <div className="fixed inset-0 z-50 bg-bg-primary flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-bg-secondary border-b border-border-primary">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="px-5 py-1.5 rounded-lg text-sm font-medium bg-accent-primary text-white hover:bg-accent-primary/90 disabled:opacity-40 disabled:cursor-not-allowed"
          >
      ...
      {/* Instructions banner */}
      {completedCount === 0 && (
        <div className="bg-yellow-500/10 border-b border-yellow-500/30 px-4 py-2 text-center text-sm text-yellow-600">
          Click on each highlighted field to fill in your signature, initials, or other information
        </div>
      )}

      {/* PDF viewer */}
      <div className="flex-1 overflow-auto p-6 bg-bg-tertiary/50 flex justify-center">
        <PdfViewer
          pdfData={pdfData}
      ...
      <div className="absolute right-4 top-20 w-52 bg-bg-secondary rounded-lg shadow-lg border border-border-primary p-3 max-h-[60vh] overflow-y-auto">
          ...
          return (
            ...
            >
              <span className={`w-4 h-4 rounded-full border flex items-center justify-center text-xs ${
                isComplete ? 'border-green-500 bg-green-500 text-white' : 'border-border-primary'
              }`}>
                {isComplete ? '✓' : ''}
              </span>
