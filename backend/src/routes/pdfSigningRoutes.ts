...
router.post('/upload', upload.single('file'), async (req: Request, res: Response) => {
  try {
    ...
    const { passphrase, description, signerConnectionId } = req.body;
    const file = req.file;
    ...
    if (signerConnectionId) {
      // Create a signing vault encrypted to the signer's KEM key
      result = await agent.modules.vaults.createSigningVault({
        document: pdfBytes,
        signerConnectionId,
        documentType: 'pdf',
        metadata: {
      ...
      // Create a regular vault with passphrase encryption
      result = await agent.modules.vaults.create(pdfBytes, {
        passphrase,
        metadata: {
          filename: file.originalname,
          mimeType: file.mimetype,
...
router.post('/sign/:vaultId', async (req: Request, res: Response) => {
  try {
    ...
    // Get the original vault info for metadata
...
router.post('/download/:vaultId', async (req: Request, res: Response) => {
  try {
    ...
    const filename = vaultInfo.metadata?.filename || 'document.pdf';
...
router.post('/return/:vaultId', async (req: Request, res: Response) => {
  try {
    ...
    const result = await agent.modules.vaults.returnSignedDocument({
      ...
      originalVaultId: vaultInfo.metadata?.originalVaultId || vaultId,
      ownerConnectionId,
      documentType: 'pdf',
      metadata: {
        ...vaultInfo.metadata,
...
router.get('/status', async (req: Request, res: Response) => {
  try {
    ...
    const pdfVaults = allVaults.filter((v: any) =>
      v.metadata?.mimeType === 'application/pdf' ||
      v.metadata?.documentType === 'pdf'
    );

    // Categorize by status
    const pending = pdfVaults.filter((v: any) => !v.metadata?.isSigned && !v.metadata?.returnedAt);
    const signed = pdfVaults.filter((v: any) => v.metadata?.isSigned && !v.metadata?.returnedAt);
    const completed = pdfVaults.filter((v: any) => v.metadata?.returnedAt);
    ...
    res.json({
      ...
      vaults: {
        ...
        signed: signed.map((v: any) => ({
          vaultId: v.vaultId,
          filename: v.metadata?.filename,
          signedAt: v.metadata?.signedAt,
        })),
        completed: completed.map((v: any) => ({
          vaultId: v.vaultId,
          filename: v.metadata?.filename,
          returnedAt: v.metadata?.returnedAt,
        })),
      },
    });
  } catch (error: any) {
